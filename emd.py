...
def check_shape(event):
    assert event.ndim == 2 and event.shape[-1] == 3, "Event shape must be (n, 3)"
...
def emd_pot(source_event, target_event, norm=False, R=1.0, return_flow=False, n_iter_max=100000):
    ...
    # implement the EMD in Eq. 1 of the paper by adding an appropriate extra particle
        ...
        if pTdiff > 0:
            ...
            coords0_extra = np.vstack((source_coords, np.zeros(source_coords.shape[1], dtype=np.float64)))
            thetas = cdist(coords0_extra, target_coords)/R
            thetas[-1,:] = 1.0

        elif pTdiff < 0:
            target_pTs = np.hstack((target_pTs, -pTdiff))
            coords1_extra = np.vstack((target_coords, np.zeros(target_coords.shape[1], dtype=np.float64)))
            thetas = cdist(source_coords, coords1_extra)/R
            thetas[:,-1] = 1.0
